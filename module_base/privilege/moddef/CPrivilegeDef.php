...
class CPrivilegeDef extends \modbase\core\CModDef {
	...
	protected function desc() {
		return array(
			...
			self::FTR => array(
				'privFtr' => array(self::G_CS => '\modbase\privilege\CPrivFilter', 
				    self::G_DC => '对当前用户的访问权限进行判断和过滤'),
			),
			self::TBDEF => array(
				'priv_group' => "(
				...
				'priv_user' => "(
			        ...
			        note          varchar(16) not null  default '',
					unique key(user_id),
					key(priv_group_id)
				)"
			),
		...
		try {
		    ...
		    if(empty($ret)){
    			...
    			$pgid = $ins->add(array(
    			    ...
    			    'name'      => 'dev',
    			    'type'      => self::TYPE_ALLOW,
    			    'priv_list' => CPrivGroupControl::encodePrivList(array(
    			        'core'=>array('action_info', 'api_log'),
    			    )),
    			    'create_ts' => time()
    			));
    			$ins = CPrivUserControl::getInstance(self::$appenv, $dbpool, $mempool, 2);
    			$ins->setSecondKey(10);
    			$ins->delNode();
    			$ins->add(array(
    			    'user_id'       => 10,
