...
function fileUpload(opt){
	...
	var _show = function(file){
		...
		bind(_win, 'click', function(e){if(opt.onFileDel) opt.onFileDel(file, function(){_remove(_win);});else _remove(_win);});
		num_of_files++;
	}
	var _edit = function(inpf){
		cntr.appendChild(inpf);
		inpf.style.display = "none";
		var img = document.createElement("img");
		if(inpf.files && inpf.files[0]){
			img.src = window.URL.createObjectURL(inpf.files[0]);
...
function popwin(title, body){
	...
	pw.innerHTML = "<div class=pwtitle><span></span><a href='javascript:;'>&times</a></div><div class=bd></div>";
	pw._title = function(t){pw.childNodes[0].childNodes[0].innerHTML = t;return pw;}
	pw.body=function(b){var bd=pw.childNodes[1];if(0==arguments.length) return bd; bd.innerHTML="";if("object" == typeof b) bd.appendChild(b); else bd.innerHTML = b;return pw;}
	pw.show=function(){pw.style.display="block";pw.bdheight();if(mask_win) mask_win.style.display="block";return pw;}
	pw.hide=function(){pw.onclose();pw.style.display="none";if(mask_win) mask_win.style.display="none";return pw;}
	...
	pw.note=function(node){if(-1 == pw.className.indexOf("popwin-note")) pw.className += " popwin-note";pw.around(node).noclose().autoclose();return pw;}
	pw._title(title);
	pw.body(body);
	pw.childNodes[0].getElementsByTagName("a")[0].onclick=function(e){pw.hide();}
