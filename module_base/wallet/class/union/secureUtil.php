...
function sign(&$params) {
	...
	unpay_log( "签名key=val&...串 >" . $params_str );
	
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	unpay_log ( "摘要sha1x16 >" . $params_sha1x16 );
	// 签名证书路径
	...
	if ($sign_falg) {
		...
		unpay_log ( "签名串为 >" . $signature_base64 );
...
function verify($params) {
	...
	$params_str = http_build_query ( $params );
	unpay_log( '报文去[signature] key=val&串>' . $params_str );
	$signature = base64_decode ( $signature_str );
//	echo date('Y-m-d',time());
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	unpay_log( '摘要shax16>' . $params_sha1x16 );	
	$isSuccess = openssl_verify ( $params_sha1x16, $signature,$public_key, OPENSSL_ALGO_SHA1 );
	unpay_log( $isSuccess ? '验签成功' : '验签失败' );
	return $isSuccess;
}
...
function getPulbicKeyByCertId($certId) {
	unpay_log ( '报文返回的证书ID>' . $certId );
	// 证书目录
	$cert_dir = SDK_VERIFY_CERT_DIR;
	unpay_log ( '验证签名证书目录 :>' . $cert_dir );
	...
	if ($handle) {
		...
		unpay_log( '没有找到证书ID为[' . $certId . ']的证书' );
	} else {
		$unpay_log ( '证书目录 ' . $cert_dir . '不正确' );
	}
	closedir ( $handle );
...
function EncryptedPin($sPin, $sCardNo ,$sCrt){
