...
mbs_import('common', 'CImage', 'CCommonCityDictCtr');
...
if(empty($info['city_no'])) $output['userinfo']['city_name'] = '';
else{
    $city = CCommonCityDictCtr::getInstance($mbs_appenv,
        CDbPool::getInstance(), CMemcachedPool::getInstance(), $info['city_no']);
    $c = $city->get();
    $output['userinfo']['city_name'] = empty($c) ? '' : $c['name'];
}
