...
class CUserDef extends \modbase\core\CModDef {
	protected function desc() {
		return array(
			...
			self::FTR => array(
				'checkLogin' =>array(self::G_CS=>'\modbase\user\CUserSession', self::G_DC=>'检查用户是否登录'),
			),
			...
			self::TBDEF => array(
				...
				'user_info' => '(
			        ...
			        career_cid           int unsigned not null default 0,
			        city_no              int unsigned not null default 0,
				    reg_ts               int unsigned not null default 0,
				    reg_ip               varchar(32) not null default "",
					pwd_modify_count     int unsigned not null default 0,
			        status               tinyint not null default 0,
			        gender               tinyint not null default 0, -- 0: sceret, 1: male, 2: female
					primary key(id),
					unique key(phone)
				)',
			    'user_device' => '(
			        uid      int unsigned not null default 0,
			        sid      char(32) CHARACTER SET latin1 NOT NULL default "", -- session id
			        type     tinyint NOT NULL default 0,   -- PC, PAD, PHONE, ...
			        os       tinyint NOT NULL default 0,     -- windows, ios, android, ..
			        ip       varchar(64) CHARACTER SET latin1 NOT NULL default "",
			        token    varchar(32) CHARACTER SET latin1 NOT NULL default "",  -- device id(for pushing)
			        login_ts int unsigned not null default 0,
			        status   tinyint NOT NULL default 0, -- 0: online, 1:offline
			        primary key(uid)
			    )',
			),
			self::PAGES => array(
				...
				'login' => array(
					...
					self::P_ARGS => array(
					    ...
					    'device_token'  => array(self::PA_REQ=>0, self::PA_EMP=>0, self::G_DC=>'设备号', self::PA_RNG=>'16, 33'),
					    'device_desc'   => array(self::PA_REQ=>0, self::PA_EMP=>0, self::G_DC=>'设备描述', self::PA_RNG=>', 128'),
					),
				...
				'edit' => array(
					...
					self::P_ARGS => array(
					    ...
					    'gender'       => array(self::PA_REQ=>0, self::G_DC=>'性别', self::PA_TYP=>'integer'),
					    'career_cid'   => array(self::PA_REQ=>0, self::G_DC=>'职业', self::PA_TYP=>'integer'),
					),
				),
				'list' => array(
					self::P_TLE => '用户管理',
				...
				'myinfo'  => array(
					...
					self::P_OUT => 'data:{userinfo:{user-info, city_name:""}[, gender_list:{0:**,1:**}, career_list:{0:**, 1:**}]}',
					self::LD_FTR => array(
