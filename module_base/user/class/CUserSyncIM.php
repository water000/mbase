...
class CUserSyncIM{ 
    ...
    static function add($user, $ch, $opts){
        ...
        $params = array(
            ...
            'name'     => isset($_REQUEST['name'])&&!empty($_REQUEST['name'])?$_REQUEST['name'] 
                : substr($_REQUEST['phone'], 0, 3).'*****'.substr($_REQUEST['phone'], 8, 3),
            '_ts'      => time(),
        );
    ...
    static function edit($diff, $ch, $opts){
        ...
        if(isset($diff['password']) && (isset($_REQUEST['pwd1']) || isset($_REQUEST['password']))){
        ...
        if(isset($_FILES['avatar']) && UPLOAD_ERR_OK == $_FILES['avatar']['error']){
            ...
            if($jret != NULL && 'success' == $jret['status']){
                $im_user['avatar'] = $jret['real_path'];
            }else{
                trigger_error('im_user_upload_err:'.$ret, E_USER_WARNING);
            }
        }
