<?php 
use modbase\core\CDBLogAPI,
    modbase\common\CDbPool;

$mbs_appenv->setLogAPI(null); // close the log in current page

$dblog = new CDBLogAPI(CDbPool::getInstance());

if(isset($_REQUEST['timeline'])){
    $ret = $dblog->read(intval($_REQUEST['timeline']), 50);
    $out = array();
...
function _draw(list){
	...
	for(var i=0; i<list.length; i++){
		...
		ul.childNodes[2].onclick = function(e){open('','', 'width=600,height=400').document.write(this.innerHTML);}
	}
}
function _clear(){
