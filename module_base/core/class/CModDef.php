...
abstract class CModDef {
	...
	static function _createClass($class, $tbname, $tbdef, $path, $mod){
	    static $uniq = 
'namespace modbase\#modname#;
...
class #classname# extends CMixRowControl {
	        ...
	        if(preg_match('/--\s+(U|M)\(([^\)]+)\)/', $def, $matches) > 0){
	            ...
	            if('U' == $matches[1]){
	                file_put_contents($path, "<?php\r\n".
	                    str_replace(array('#classname#', '#tbname#', '#keyname#', '#modname#'),
	                        array($class, $tbname, trim($keys[0]), $mod), $uniq).
	                    "\r\n?>");
	            }else{
	                $two = explode('|', trim($keys[0]));
	                if(2 == count($two)){
	                    file_put_contents($path, "<?php\r\n".
	                        str_replace(array('#classname#', '#tbname#', '#keyname#', '#mkeyname#', '#secondkeyname#', '#modname#'),
	                            array($class, $tbname, trim($two[0]), trim($two[1]), trim($keys[1]), $mod), $mix).
	                        "\r\n?>");
	                }else{
	                    file_put_contents($path, "<?php\r\n".
	                        str_replace(array('#classname#', '#tbname#', '#keyname#', '#secondkeyname#', '#modname#'),
	                            array($class, $tbname, trim($keys[0]), trim($keys[1]), $mod), $multi).
	                        "\r\n?>");
	                }
	            }
	...
	function init(){
	    if(isset($this->desc[self::TBDEF])){
	        $dir = self::$appenv->getDir($this->desc[self::MOD][self::G_NM], \modbase\CAppEnv::FT_CLASS);
	        if(!file_exists($dir) && !mkdir($dir)){
    	        ...
    	        foreach($this->desc[self::TBDEF] as $tbn => $tbdef){
    	            ...
    	            if(!file_exists($classpath)){
    	                self::_createClass($class, $tbn, $tbdef, $classpath, $this->desc[self::MOD][self::G_NM]);
    	            }
    	        }
	        }
	    }
	    if(isset($this->desc[self::PAGES])){
	        $dir = self::$appenv->getDir($this->desc[self::MOD][self::G_NM], \modbase\CAppEnv::FT_ACTION);
	        if(!file_exists($dir) && !mkdir($dir)){
	             trigger_error(sprintf('[error]mkdir "%s" failed on "%s:%d".'), $dir, __FILE__, __LINE__);
	        }else{
	    ...
	    $config_dir = self::$appenv->getDir($this->desc[self::MOD][self::G_NM], \modbase\CAppEnv::FT_CONFIG);
	...
	static function pargRange($range, &$s, &$e=0){
		$ret = 1;
		$arr = explode(',', $range);
		$s = intval(trim($arr[0]));
		if(isset($arr[1])){
			...
			$defopts = array(
	 		...
	 		foreach($pargs as $name => $opts){
				...
				if(!empty($opts[CModDef::PA_RNG]) && isset($_REQUEST[$name]) && !empty($_REQUEST[$name])){
					...
					$s = $e = 0;
					$rnum = self::pargRange($opts[self::PA_RNG], $s, $e);
	...
	function echofldhtml($name, $action=''){
	    $def = $this->item(self::PAGES, 
	        empty($action) ? self::$mbs_appenv->item('cur_action') : $action, 
	        self::P_ARGS, 
	        $name);
	    echo $def[self::G_TL],
	       isset($def[self::PA_REQ]) && $def[self::PA_REQ] ? '<span class=required>*</span>':'';
	}
	
	function echofldrange($name, $action=''){
	    $range = $this->item(self::PAGES, 
	        empty($action) ? self::$mbs_appenv->item('cur_action') : $action, 
	        self::P_ARGS, 
	        $name,
	        self::PA_RNG);
	    $s = $e = 0;
	    $rnum = self::pargRange($range, $s, $e);
	    echo $s, 2==$rnum && $e>0?'~'.$e:'',
    	    self::$mbs_appenv->lang('num_of_char', 'common'),
    	    2==$rnum && 0 == $e ? self::$mbs_appenv->lang('above') : '',
    	    '.';
	}
	...
	function installTables($dbpool, $tabledef){
		...
		foreach($tabledef as $name => $def){
			$ret = $pdoconn->exec(sprintf('CREATE TABLE IF NOT EXISTS %s%s CHARACTER SET=%s, Engine=innodb',
					mbs_tbname($name), $def,
