...
class CStrTools {
	...
	static function cutstr($str, $maxlen, $charset, &$reallen, $suffix='...'){
		$len = iconv_strlen($str, $charset);
		if($len > $maxlen){ 
		    $reallen = $maxlen;
			return iconv_substr($str, 0, $maxlen, $charset).$suffix;
		}else{
		    $reallen = $len;
		    return $str; 
		}
	...
	static function secdesc($sec){
	    static $unit = array('s', 'm', 'h');
	    
	    $sec = floatval($sec);
	    for($i=0; $i<count($unit) && $sec>60; ++$i){
	        $sec /= 60;
	    }
	    
	    return array(sprintf('%.2f', $sec), $unit[$i==count($unit) ? $i-1 : $i]);
	}
