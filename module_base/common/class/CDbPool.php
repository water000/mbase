...
class CDbPool implements IDebugOutput
{
	...
	function cli()
	{
		...
		echo sprintf("DB-total: %d quer(ies), cost:%f\n", $queryNum, $rConn->getTotalExec());
	}
	...
	public function createdb()
	{
	    try
	    {
    	    foreach(self::$arrConfig as $k => $conf)
    	    {
    	        list($host, $port, $dbname) = explode(self::SEP_HOST_PORT, $k, 3);
    	        $sDSN = sprintf('mysql:host=%s;port=%d;%s', // Note: This only works as of PHP 5.3.6.
    	            $host, $port, empty(self::$charset)?'':'charset='.str_replace('-', '',self::$charset));
	            if($this->sClassName == self::CLASS_PDODEBUG)
	                $obj = new CPDODebug($sDSN, $conf['username'], $conf['pwd'], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
	            else
	                $obj = new PDO($sDSN, $conf['username'], $conf['pwd'], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
	            $obj->query('CREATE DATABASE IF NOT EXISTS '.$dbname);
    	    }
	    }
	...
	public function getDefaultConnection()
	{
		...
		return $this->getConnection($host, $port, $dbname);
	}
	...
	public function getConnection($sHost, $port, $sDbName)
	{
		...
		if($this->sClassName == self::CLASS_PDODEBUG)
			$this->arrConnDebug[$sKey] = $obj =
				 new CPDODebug($sDSN, $conf['username'], $conf['pwd'], array(
				     \PDO::ATTR_ERRMODE => \PDO::ERRMODE_EXCEPTION,
				     \PDO::MYSQL_ATTR_LOCAL_INFILE => true
				 ));
		else
			$obj = new \PDO($sDSN, $conf['username'], $conf['pwd'], array(
			    \PDO::ATTR_ERRMODE => \PDO::ERRMODE_EXCEPTION,
			    \PDO::MYSQL_ATTR_LOCAL_INFILE => true
			));
